// Copyright (C) 2011 - wbxie.  All rights reserved.
...
public class Command<K, V, T> implements Future<T> {
    ...
    public final RedisCommand type;
    protected CommandArgs<K, V> args;
    protected CommandOut<K, V, T> output;
     ...
     */
    public Command(RedisCommand type, CommandOut<K, V, T> output, CommandArgs<K, V> args, boolean multi) {
        this.type   = type;
        this.output = output;
        this.args   = args;
     ...
     */
    public CommandOut<K, V, T> getOutput() {
        return output;
    }
